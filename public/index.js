    ...
    viewportNavigator.onmousedown = () => {
      ...
      document.body.onmousemove = (e) => {
          ...
          if(lastPos) {
            ...
            pdfViewer.scrollTop = pdfViewer.scrollTop + (((delta*viewportNavigator.offsetHeight)/MINIMAP_SCALE) / (viewportNavigator.offsetHeight*MINIMAP_SCALE));
        }
  ...
  Array.from($caseFacetsTableBody.querySelectorAll("tr")).forEach(tr => {
    ...
    if (rowFacet && rowFacet.value != null) {
      ...
      if(rowFacet.type == "date") {
        ...
        let dateStr = `${rowFacet.value[0] ? rowFacet.value[0] + "-" : ""}${months[rowFacet.value[1]-1]}-${rowFacet.value[2]}`
...
window.onload = async () => {
  ...
  $caseSearch.onkeyup = async (e) => {
    ...
    searchDebounce = setTimeout(async () => {
      ...
      const pdfMinimapEls = processPages(pdfMinimapInner, searchResultsWithBoundingBoxes.boundingBoxes, MINIMAP_SCALE, 'searchDivPoint');
